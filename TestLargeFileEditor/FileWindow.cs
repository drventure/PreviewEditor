...
namespace TestLargeFileEditor
{
    internal class FileWindow: IDisposable
    {
        ...
        private string getWindow(long? windowOffset = null)
        {
            ...
            _windowOffset = long.Max(_windowOffset - _windowSize / 2, 0);
            //constrain to no more than EOF - the windowsize
            _windowOffset = long.Min(_windowOffset, _file.Length - _windowSize + 1);
        ...
        private long resolveOffset(long? offset)
        {
            if (offset.HasValue)
            {
                if (offset >= 0)
                {
                    ...
                    _windowOffset = long.Min(offset.Value, _file.Length);
                }
                else
                {
                    //offset from EOF (where -1 means EOF, so we have to offset by one
                    _windowOffset = _file.Length - long.Max(offset.Value, -_file.Length) + 1;
                }
            }
        ...
        private void CleanBuffer(Span<byte> byt)
        {
